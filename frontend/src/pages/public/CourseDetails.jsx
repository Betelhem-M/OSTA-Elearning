...
import { Clock3, Languages, Timer } from 'lucide-react'
  ...
  if (error || !course) {
    return (
      <div className="mx-auto max-w-[600px] px-5 py-16 text-center">
        ...
        <p className="mt-2 text-sm text-slate-500">{error || `"${courseId}" doesn't match any course.`}</p>
    ...
    <main className="mx-auto max-w-[1100px] px-5 py-8 lg:px-10">
      ...
      <div className="mt-6 grid gap-8 lg:grid-cols-[1.6fr_1fr]">
        <div className="space-y-8">
          <div>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-3">
              <div className="rounded-xl border border-slate-100 bg-white p-4 shadow-sm">
                ...
                <p className="mt-2 text-sm font-bold text-ink">{course.language || 'English'}</p>
              </div>
              ...
              <div className="rounded-xl border border-slate-100 bg-white p-4 shadow-sm">
                ...
                <p className="mt-2 text-sm font-bold text-ink">{course.duration || 'Self-paced'}</p>
              </div>

              <div className="rounded-xl border border-slate-100 bg-white p-4 shadow-sm">
                <div className="flex items-center gap-2 text-xs font-semibold text-slate-400">
                  <Timer size={15} /> Learning Time
                </div>
                <p className="mt-2 text-sm font-bold text-ink">
                  {hasHours ? `${estimatedHours} ${estimatedHours === 1 ? 'hour' : 'hours'}` : 'Not specified'}
                </p>
              </div>
            </div>
          ...
          {curriculumLoading ? (
            <section>
              <div className="mb-3"><h2 className="text-lg font-bold text-ink">Course Curriculum</h2></div>
              <div className="rounded-xl border border-slate-100 bg-slate-50 px-5 py-6 text-center text-sm text-slate-400">Loading curriculum...</div>
            </section>
          ) : (
      ...
      <section className="mt-12">
        ...
        <div className="grid gap-5 sm:grid-cols-3">
          {courses
            .filter((item) => Number(item.id) !== Number(course.id) && Number(item.category_id) === Number(course.category_id))
            .slice(0, 3)
            .map((item) => (
              <Link
                key={item.id}
              ...
              >
                <div className="h-20 w-full rounded-lg" style={{ backgroundColor: item.thumbnail_color || '#2E7D32' }} />
