...
export default function CourseDetails() {
  ...
  useEffect(() => {
    async function loadCourse() {
      try {
        ...
        const [courseResponse, coursesResponse] = await Promise.all([fetch(`${API_URL}/courses/${courseId}`), fetch(`${API_URL}/courses`)])
        const courseData = await courseResponse.json(); const coursesData = await coursesResponse.json()
        if (!courseResponse.ok) throw new Error(courseData.message || 'Failed to fetch course')
        if (!coursesResponse.ok) throw new Error(coursesData.message || 'Failed to fetch courses')
        setCourse(courseData); setCourses(coursesData)
      } catch (err) { console.error('Course details error:', err); setError(err.message || 'Failed to load course') }
  ...
  useEffect(() => {
    async function loadCurriculum() {
      try { setCurriculumLoading(true); const response = await fetch(`${API_URL}/course-sections/course/${courseId}`); const data = await response.json(); if (!response.ok) throw new Error(data.message || 'Failed to fetch course curriculum'); setCurriculum(Array.isArray(data) ? data : []) }
      catch (err) { console.error('Curriculum error:', err); setCurriculum([]) }
      finally { setCurriculumLoading(false) }
    }
    loadCurriculum()
  }, [courseId])

  if (loading) return <main className="mx-auto max-w-[1100px] px-5 py-16 lg:px-10"><div className="text-center text-sm font-semibold text-slate-500">Loading course...</div></main>
  if (error || !course) return <div className="mx-auto max-w-[600px] px-5 py-16 text-center"><h1 className="text-xl font-bold text-ink">Course not found</h1><p className="mt-2 text-sm text-slate-500">{error || `"${courseId}" doesn't match any course.`}</p><Link to="/courses" className="mt-4 inline-block text-sm font-bold text-primary hover:underline">Back to Course Marketplace</Link></div>
