...
import { Share2, ChevronRight, CheckCircle2, AlertCircle, ArrowLeft } from "lucide-react";
  ...
  return <div className="mx-auto max-w-[1400px] px-4 py-6 lg:px-8">
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1.7fr)_minmax(340px,0.8fr)]">
      <div className="space-y-4">
        ...
        <section className="rounded-xl border border-slate-100 bg-white p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-lg font-bold text-ink">{lesson.title}</h1><p className="mt-1 text-xs text-slate-400">{Math.round(progressPercent)}{t.watched}</p></div><div className="flex gap-2"><BookmarkButton contentType="lesson" contentId={lesson.id} className="rounded-lg border border-slate-200 bg-transparent shadow-none text-slate-600 hover:bg-slate-50"/><button onClick={handleShare} className="rounded-lg border border-slate-200 p-2 text-slate-600"><Share2 size={15}/></button></div></div>{copyStatus === "copied" && <p className="mt-2 text-right text-xs text-green-600">{t.copied}</p>}{isSavingProgress && <p className="mt-2 text-right text-xs text-slate-400">{t.saving}</p>}<div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-3"><span className="text-xs text-slate-400">{lesson.section_title}</span><button onClick={handleCompleteLesson} disabled={completed} className={`inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-bold ${completed ? "bg-primary-light text-primary" : "bg-primary text-white hover:bg-primary-hover"}`}><CheckCircle2 size={14}/>{completed ? t.completed : t.mark}</button></div></section>
