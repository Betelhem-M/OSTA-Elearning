...
const AuthToken={
 ...
 async isVerified(userId){const [[row]]=await pool.execute(`SELECT verified_at FROM email_verification_codes WHERE user_id=? ORDER BY id DESC LIMIT 1`,[userId]);return Boolean(row?.verified_at);},
